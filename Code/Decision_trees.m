...
Tree_Pe_train=sum(Labels_train ~= outputs_train)/length(Labels_train); 
CM_Train=confusionmat(Labels_train,outputs_train);
% Measure Val error
outputs_val = predict(tree,X_val);
Tree_Pe_val=sum(Labels_val ~= outputs_val)/length(Labels_val); 
CM_Val=confusionmat(Labels_val,outputs_val);
